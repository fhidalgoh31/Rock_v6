...
byte parameters[7]={   0,    10,      2,         2,         5,          1,       2};
  ...
  case 1:
      ...
      { 
        if (gf)
          ...
          {
            Serial.print("GF N-->Burst RB, GPS ON...");
            ol_logevent(2);
            command('g');  //GPS ON
...
////////////////  New message every time
            ...
            Serial.println("XXX wait to turn on:");
            delay(10);
            rck_on_off(1);   // start Rockblock
            myRock.listen();

            Serial.println("XXX start clear buffer:");
            int err=isbd.clear_buffer(2);
            if (err != 0)
              {
                Serial.print("Clear Buffer failed: error ");
                Serial.println(err);
              }else
              {
                Serial.print("Clear Buffer Successfully ");
              }
            delay(100);
...
///////////
    ...
    if ((now()> temp1) && (now()> temp2))  //   evaluate overall time out
      {
        Serial.println("No signal going to small_sleep");
  ...
  case 5:
      if (now()> temp1)  //   evaluate overall time out
      {
